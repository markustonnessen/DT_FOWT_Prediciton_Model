...
- `MLSTM_predictor.py`: This file receives measured wave elevation and system states, and sends them into `dolphinn.py` in the MLSTM model. It also receives predicted system states from `p2v.py`. It is also responsible for making the single `Prediction_state` plot ready to be visualized and making `.csv` of prediction data ready for saving within the `Prediction_functions.py`.
- `MLSTM_batching.py`: This file defines the PredictonClass, which collects simulation data made by `Sima_output_converter.py` and builds batches of measured data used for prediction. The desired batch size is determined within this file, with `self.batch_size = 1000` for example. When `len(self.batch_data) >= batch_size` and `current_time \% pred_freq == 0` the prediction is triggered.
