...
def run_DOLPHINN(data_frame_inputs, DOLPHINN_PATH, plot_figure, current_time, pred_error_x, pred_error_y, save_csv, save_csv_time, FOWT_pred_state, sim_length, required_measurements, data_source):
    global prediction_history
    
    # Load the trained model
    dol = DOL()
    # print(f"[DEBUG] Calling DOL.load() with: {DOLPHINN_PATH}")
    ...
    if plot_figure:
        active_pred_plot(t_pred, y_hat, pred_error_x, pred_error_y, data_frame_inputs, current_time, dol, time_data, t1_idx, t2, t1, fig, ax, FOWT_pred_state)
    ...
    if current_time == save_csv_time and save_csv:
        ...
        data_source  # <- You might need to pass this into DOLPHINN or provide it
    )
