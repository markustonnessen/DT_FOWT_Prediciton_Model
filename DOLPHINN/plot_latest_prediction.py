...
# === CONFIGURATION ===
prediction_dir = os.path.join(os.path.dirname(__file__), "prediction_results")
font_props = FontProperties(size=8)  # Smaller font for compact legends
...
if "Floater" in measurement_filename:
    ...
    sim_type = "SOV"
elif "Gangway" in measurement_filename:
    sim_type = "Gangway"
...
if sim_type == "Gangway":
    ...
    fig.suptitle("Gangway Predicted Motions and Velocities with Thresholds", fontsize=15)
    ...
    for i, param in enumerate(shared_params):
        ...
        axs[i].plot(df_pred["Time"], df_pred[f"Predicted_{param}"], label=f"Predicted {param}", color="green")
        axs[i].set_ylabel(f"{param}{param_units.get(param, '')}")
        axs[i].legend(loc='center left', bbox_to_anchor=(1.0, 0.5))
        axs[i].grid(True)
        axs[i].set_xlim(xlim_min, xlim_max)
    ...
    axs[-1].set_xlabel("Time [s]")
    fig.suptitle(f"{sim_type} Prediction vs. Measurements SIMA", fontsize=15)

plt.tight_layout(rect=[0, 0, 1, 0.96])
plot_path = os.path.join(prediction_dir, f"PredictionPlot_{sim_type}_{save_time_str}s.png")
...
if can_compute_gangway:
    ...
    fig2.suptitle("Predicted Gangway Motions and Velocities (computed from DOFs)", fontsize=15)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    path2 = os.path.join(prediction_dir, f"GangwayComparison_{sim_type}_{save_time_str}s.png")
    plt.savefig(path2, dpi=300)

    df_gangway_pred.to_csv(os.path.join(prediction_dir, f"PredictedGangwayMotionsAndVelocities_{sim_type}_{save_time_str}s.csv"), index=False)
    if has_measured:
        df_gangway_meas.to_csv(os.path.join(prediction_dir, f"MeasuredGangwayMotionsAndVelocities_{sim_type}_{save_time_str}s.csv"), index=False)
