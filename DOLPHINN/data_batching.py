...
class PredictionClass():
    ...
    def run_simulation(self, current_time, measurements, plot_figure, time_horizon, pred_error_x, pred_error_y, pred_freq, save_csv, save_csv_time, FUTURE_WAVE_FILE, FOWT_pred_state, MLSTM_MODEL_NAME, DOLPHINN_PATH, sim_length, required_measurements, data_source):
    
        from wave_predict import run_DOLPHINN
        ...
        if len(self.batch_data) >= self.batch_size and current_time % pred_freq == 0:
            ...
            FOWT_pred_state, sim_length, required_measurements, data_source
            )
        ...
        if current_time == save_csv_time and save_csv:
            ...
            full_output_file_path = os.path.join(self.this_dir, "prediction_results", f"measurements_{current_time}_{source_tag}_ACTIVE.csv")
