...
def extract_and_save_csv(h5_input_arg, timestep):
    ...
    wave_file_path_Floater = os.path.join(incoming_path, f"{math.ceil(sim_length)}s_wave_Floater.csv")
    motion_file_path_Floater = os.path.join(incoming_path, f"{math.ceil(sim_length)}s_motion_Floater.csv")

    wave_file_path_SOV = os.path.join(incoming_path, f"{math.ceil(sim_length)}s_wave_SOV.csv")
    motion_file_path_SOV = os.path.join(incoming_path, f"{math.ceil(sim_length)}s_motion_SOV.csv")
    ...
    slewing = slewing_angle #+ df_motion_SOV['PtfmRDZ_SOV']
    ...
    wave_file_path_Gangway = os.path.join(incoming_path, f"{math.ceil(sim_length)}s_wave_Gangway.csv")
    df_wave_Gangway.to_csv(wave_file_path_Gangway, index=False)

    motion_file_path_Gangway = os.path.join(incoming_path, f"{math.ceil(sim_length)}s_motion_Gangway.csv")
    df_motion_Gangway.to_csv(motion_file_path_Gangway, index=False)
    print(f"[INFO] Saved GANGWAY wave CSV to {wave_file_path_Gangway}")
    print(f"[INFO] Saved GANGWAY motion CSV to {motion_file_path_Gangway}")

    # === COMBINE FLOATER AND SOV INTO FLOSOV ===
    # df_motion_FLOSOV = pd.concat([
    #     df_motion_Floater.drop(columns=['Time']),
    #     df_motion_SOV.drop(columns=['Time'])
    # ], axis=1)
    # df_motion_FLOSOV.insert(0, 'wave', wave_Floater)
    # df_motion_FLOSOV.insert(0, 'Time', time_array)
    ...
    # # Save combined FLOSOV motion file
    # motion_file_path_FLOSOV = os.path.join(incoming_path, f"{math.ceil(sim_length)}s_motion_FLOSOV.csv")
    # df_motion_FLOSOV.to_csv(motion_file_path_FLOSOV, index=False)
    # print(f"[INFO] Saved FLOSOV motion CSV to {motion_file_path_FLOSOV}")
